...
function main() {
	var argument1 = process.argv.splice(2);
	//console.log(argument1);
	// 解码
	if (argument1[0].toString() == "-d") {
		var arguments = argument1[1].toString();
		if (!arguments) {
		...
		try {
			let temp = fs.readFileSync(arguments, 'utf8');
			console.log("\n---------JJcoder by rain---------\nOutput:\n");
			var jstxt;
			jstxt = jjdecode(temp);
			var outfile = arguments.toString().split(".")[0] + "-decode-result.js";
			console.log(jstxt);
			fs.writeFile(outfile, jstxt, (err) => {
				if (err) {
	...
	//编码
	if (argument1[0].toString() == "-e") {
		// 文件名
		var arguments = argument1[2].toString();

		// 密钥
		var code = argument1[1];
		//console.log(argument1);
		if (!arguments || argument1.length != 3) {
			console.log("Parameter error!");
			return;
		} else {
			//console.log(arguments);
		...
		try {
			...
			var jstxt = encode(code, temp);


			var outfile = arguments.toString().split(".")[0] + "-encode-result.js";
...
function jjencode(gv, text) {
	...
	var n;
	var t;
	var b = ["___", "__$", "_$_", "_$$", "$__", "$_$", "$$_", "$$$", "$___", "$__$", "$_$_", "$_$$", "$$__", "$$_$",
		"$$$_", "$$$$",
	];
	var s = "";
	for (var i = 0; i < text.length; i++) {
		...
		} else if ((0x21 <= n && n <= 0x2f) || (0x3A <= n && n <= 0x40) || (0x5b <= n && n <= 0x60) || (0x7b <= n &&
				n <= 0x7f)) {
	...
	r =
		...
		gv + ",$_$$:({}+\"\")[" + gv + "],$$_$:(" + gv + "[" + gv + "]+\"\")[" + gv + "],_$$:++" + gv +
		",$$$_:(!\"\"+\"\")[" +
		gv + "],$__:++" + gv + ",$_$:++" + gv + ",$$__:({}+\"\")[" + gv + "],$$_:++" + gv + ",$$$:++" + gv +
		",$___:++" + gv + ",$__$:++" + gv + "};" +
		gv + ".$_=" +
		"(" + gv + ".$_=" + gv + "+\"\")[" + gv + ".$_$]+" +
		"(" + gv + "._$=" + gv + ".$_[" + gv + ".__$])+" +
...
function jjdecode(t) {
	...
	//hex decode string
	var b = ["___+", "__$+", "_$_+", "_$$+", "$__+", "$_$+", "$$_+", "$$$+", "$___+", "$__$+", "$_$_+", "$_$$+",
		"$$__+", "$$_$+", "$$$_+", "$$$$+"
	];
	//lotu
	var str_l = "(![]+\"\")[" + gv + "._$_]+";
			...
			} else //"S ----> "SR or "S+
			{
				...
				while (true) {
					...
					} else if (0 == data.indexOf(str_lower)) //r3 check if "R // n < 128
					{
						...
						out += String.fromCharCode(parseInt(ch_str, 8)) + ch_lotux;
						break; //step out of the while loop
					} else if ((0x21 <= n && n <= 0x2f) || (0x3A <= n && n <= 0x40) || (0x5b <= n && n <= 0x60) || (
							0x7b <= n && n <= 0x7f)) {
						out += data.charAt(0);
						data = data.substr(1);
						match += 1;
